...
Page({
  ...
  search:function(){
    var that = this;
    if (valid.valid.isEmputy(this.data.keyWords)){
      wx.showToast({
        title: '搜索内容不能为空',
        icon:'none',
        duration:2000
    ...
    app.POST({
      ...
      data:{
        ...
        request:'public.product.search.goods.list.get',
        token:app.globalData.session_key,
        product_name: that.data.keyWords,
        curpage:that.data.curpage
      },
      success:function(res){
        // console.log(res);
        if(res.data.code == 0){
          var sResult = that.data.sResult || [];
          sResult = sResult.concat(res.data.data.data);
          that.setData({
            sResult: sResult,
